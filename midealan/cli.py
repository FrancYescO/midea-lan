...
from midealan.discover import SERIAL_TYPE1_LENGTH, discover
...
def get_config_file_path(relative: bool = False) -> Path:
    ...
    local_path = Path("midea-lan.json")
    if relative or local_path.exists():
        return local_path
    return platformdirs.user_config_path(appname="midea-lan").joinpath(
        "midea-lan.json",
    )
...
def main() -> NoReturn:
    ...
    parser = ArgumentParser(description="Command line utility for midea-lan.")
    parser.add_argument(
        "-v",
        "--version",
        action="version",
        version=f"midea-lan version: {__version__}",
    )
